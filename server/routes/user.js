...
userRoutes.post("/api/login", async (req, res) => {
	...
	const userData = await UserData.findOne({ username: username }).lean();
	if (!user) {
		res.status(298);
...
userRoutes.delete("/api/rm-user", verifyJWT, async (req, res) => {
	...
	User.deleteOne(myquery, (err, obj) => {
		...
		UserData.deleteOne({ _id: req.user.udid }, (error, object) => {
			////////////////// below could be coded better
			console.log(JSON.stringify(object));
			if (error) {
				console.log("rm-user: " + error.message);
				res.status(298);
				res.json({ message: error.message, data: error.name });
			} else {
				res.status(200);
...
userRoutes.post("/api/update-userData", verifyJWT, async (req, res) => {
	//console.log("req.user.username: " + JSON.stringify(res.user.username));
	UserData.findOneAndUpdate(
		...
		(err, doc) => {
			if (doc) {
				res.status(200);
				res.json({ message: "Your favorites have been sucessfully updated", data: doc });
			} else {
				res.status(298);
				res.json({ message: "An Error has occured" }); //shouldnt ever execute
			}
		}
	);
});
...
userRoutes.get("/api/get-RR", verifyJWT, async (req, res) => {
	let query = req.body.name;
	Restroom.findOne(query, (err, doc) => {
		if (doc) {
			res.status(200);
			res.json({ message: "Restroom has been found", data: doc });
		} else {
			res.status(298);
			res.json({ message: "Restroom was not found in the server" });
		}
	});
});
...
userRoutes.delete("/api/rm-RR", verifyJWT, async (req, res) => {
	...
	Restroom.deleteOne(myquery, (err, doc) => {
		...
		//if no document found, then restroom doesn't exist

		if (doc.deletedCount) {
			UserData.find({ favorites: req.body.name }, (err, docs) => {
				//if no matchin guser data found, then there is no cleanup to do
				console.log("rm-rr: " + JSON.stringify(docs));
				if (docs.length) {
				...
				res.status(200);
				res.json({ message: req.body.name + " has been deleted", data: doc });
			});
		} else {
			res.json({ message: req.body.name + " is not in our database yet" });
			res.status(298);
		}
	});
});
...
userRoutes.get("/api/near-RR", async (req, res) => {
	...
	Restroom.find(myquery, (err, docs) => {
		...
		for (var i = 0; i < docs.length; i++) {
			if (
				Math.pow((docs[i].longitude - parseFloat(req.query.longitude) * 54.5833333, 2)) +
					Math.pow((docs[i].latitude - parseFloat(req.query.latitude)) * 54.5833333, 2) >
				Math.pow(parseFloat(req.query.radius), 2)
			) {
...
userRoutes.post("/api/new-review", verifyJWT, async (req, res) => {
	...
	if (!post) {
		await Review.create({
			restroomName: req.body.restroomName,
			username: req.user.username,
			time: req.body.time,
	...
	restNew.clean[0] =
		(restNew.clean[0] * restNew.clean[1] + req.body.clean) / (restNew.clean[1] + 1);
	restNew.smell[0] =
		(restNew.smell[0] * restNew.smell[1] + req.body.smell) / (restNew.smell[1] + 1);
	restNew.TP[0] = (restNew.TP[0] * restNew.TP[1] + req.body.TP) / (restNew.TP[1] + 1);
	restNew.safety[0] =
		(restNew.safety[0] * restNew.safety[1] + req.body.safety) / (restNew.safety[1] + 1);
	restNew.privacy[0] =
		(restNew.privacy[0] * restNew.privacy[1] + req.body.privacy) / (restNew.privacy[1] + 1);
	restNew.busyness[0] =
		(restNew.busyness[0] * restNew.busyness[1] + req.body.busyness) / (restNew.busyness[1] + 1);
...
userRoutes.delete("/api/del-review", verifyJWT, async (req, res) => {
	...
	rest.body.clean[0] =
		(rest.body.clean[0] * rest.body.clean[1] - rev.clean) / (rest.body.clean[1] - 1);
	rest.body.clean[1]--;
	rest.body.smell[0] =
		(rest.body.smell[0] * rest.body.smell[1] - rev.smell) / (rest.body.smell[1] - 1);
	rest.body.smell[1]--;
	rest.body.TP[0] = (rest.body.TP[0] * rest.body.TP[1] - rev.TP) / (rest.body.TP[1] - 1);
	rest.body.TP[1]--;
	rest.body.safety[0] =
		(rest.body.safety[0] * rest.body.safety[1] - rev.safety) / (rest.body.safety[1] - 1);
	rest.body.safety[1]--;
	rest.body.privacy[0] =
		(rest.body.privacy[0] * rest.body.privacy[1] - rev.privacy) / (rest.body.privacy[1] - 1);
	rest.body.privacy[1]--;
	rest.body.busyness[0] =
		(rest.body.busyness[0] * rest.body.busyness[1] - rev.busyness) / (rest.body.busyness[1] - 1);
	...
	rest.body.hygiene[0] = rest.body.hygiene - rev.hygiene;
